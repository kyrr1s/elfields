...
def read_all_fields(header_data: bytes, size: int, hex_output: bool) -> None:
...
def read_single_field(header_data: bytes, field_name: str, hex_output: bool) -> None:
    ...
    if hex_output:
        print_hex(field_data)
        sys.stdout.buffer.write(b'\n')
    else:
        print_binary(field_data)
    ...
    try:
        ...
        if args.all:
            if args.write:
                ...
                read_all_fields(header_data, size, args.hex)
        elif field_name:
            if args.write:
                write_data = hex_to_bin(args.write)
                ...
                read_single_field(header_data, field_name, args.hex)
        else:
